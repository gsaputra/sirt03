 ...
 * Mod_Novapage
 * novapage option
 * untuk konfigurasi template novapage
 * 
 * module admin
 * 
 * @package novapage
...
class Mod_Novapage extends CI_Controller {
	public function index(){         

        cek_session_akses('novapage',$this->session->id_session); 
        ...
        } elseif(isset($_POST['set_setting_footer'])){
    ...
    protected function view_index(){
         ...
         */
        $get_options  = $this->model_app->view('tbl_novapage')->result_array(); 
        ...
        $data['get_halaman_seo_dropdown'] = $this->get_halaman_seo_dropdown();
        ...
        $data['get_images'] = $this->get_images();

        $this->template->load('administrator/template','administrator/mod_novapage/view_index',$data);
    }
    ...
    protected function update_config(){ 
        ...
        if( !empty($this->input->post('sections_aktif')) && is_array( $this->input->post('sections_aktif'))) {              
            //get sections order
            $get_sections_order  = $this->model_app->view_where('tbl_novapage',array('key' => 'sections_order' ))->row_array();             
        ...
        redirect($this->uri->segment(1).'/novapage');
    }
    ...
    protected function update_urut_sections(){         
		...
		$this->session->set_flashdata('alert','Konfigurasi Telah Diupdate');
        redirect($this->uri->segment(1).'/novapage#content_section');
    }
    ...
    protected function set_group_setting_footer($section_id){
		...
		$this->session->set_flashdata('alert','Konfigurasi Telah Diupdate');
        redirect($this->uri->segment(1).'/novapage#content_'.$section_id);
    } 
    ...
    protected function save_config($key='',$value = '') {
        ...
        // get options value 
        $get_option  = $this->model_app->view_where('tbl_novapage',array('key' => $key ))->row_array(); 
     ...
     * untuk mendapatkan halaman statis
     */
    protected function get_halaman_seo_dropdown(){             
        $get_halaman = $this->db->query("
			SELECT 
                hal.judul_seo as id,
                hal.judul as judul
    ...
    /**
     * get_images
     * untuk mendapatkan asset gambar 
     */
    protected function get_images(){        
        $get_images = $this->db->query(" 
			SELECT 
                img.id_images as id,
                img.deskripsi as nama,
                img.file as file
			FROM 
                tbl_novapage_images img
			ORDER BY img.deskripsi ASC
        ")->result_array();
        
        $data_images = array();
        if(!empty($get_images)){
            $data_images = $get_images;
        } 
        return $data_images;
    }
