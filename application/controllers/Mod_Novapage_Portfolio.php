...
class Mod_Novapage_Portfolio extends CI_Controller {
	public function index(){          
        cek_session_akses('novapage-portfolio',$this->session->id_session);  
        ...
        if(isset($_POST['set_portfolio'])) {
            ...
            redirect($this->uri->segment(1).'/novapage-portfolio');
    ...
    protected function view_index(){         
        $data['get_portfolio']  = $this->model_app->view_ordering('tbl_novapage_portfolio','nama_project','ASC') ; 
        $this->template->load('administrator/template','administrator/mod_novapage_portfolio/view_index',$data);
    } 
    ...
    protected function save_data($id='',$data = array()) {
        ...
        // get data
        $get_data  = $this->model_app->view_where('tbl_novapage_portfolio',array('id_portfolio' => $id ))->row_array(); 
        ...
        if( !empty($data) && is_array($data)) { 
            if(!empty($get_data) && $get_data['id_portfolio'] == $id ){  
                ...
                return $this->model_app->insert('tbl_novapage_portfolio', $data);
            }
        } else {
            return null;
        }
    ...
    protected function upload_image($param){
        ...
        // config
        $config['upload_path'] = 'asset/img_novapage/portfolio/';
    ...
    protected function hapus_image($nama_image = '') {
        ...
        $image_file = FCPATH .'asset/img_novapage/portfolio/'.$nama_image; 
